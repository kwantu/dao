...
var dao = (function(){
  ...
  return {
    ...
    authenticateToSyncGateway: function(username, password){
      ...
      var url = 'http://' + COUCHBASE_DOMAIN +':' + COUCHBASE_PORT + '/' + TAXONOMY_BUCKET + "/_session";
      var method = "POST";
      ...
      var url = 'http://' + COUCHBASE_DOMAIN +':' + COUCHBASE_PORT + '/' + COUCHBASE_BUCKET + "/_session";
      var method = "POST";
      var postData = JSON.stringify({
        "name": username, "password": password
      });
        ...
        if (head.mobile && isCordovaApp) {
          ...
          cordova.exec(
            ...
            function(e){dfd.reject(e)},
                   'CBLite',
                   'pull',
                   [{"dbname": DBNAME + DB_IDENTIFIER, "url":'http://'+syncUsername+':'+syncPassword+'@'+COUCHBASE_DOMAIN +':' + COUCHBASE_PORT + '/' + COUCHBASE_BUCKET}]
               );
              ...
              if(authenticatedToSyncGateway == false)
              {
                ...
                .done(
                       function (data)
                       {
                           ...
                           localDB.replicate.from(new PouchDB('http://' + COUCHBASE_DOMAIN +':' + COUCHBASE_PORT + '/' + COUCHBASE_BUCKET,
                            {
                              ajax: {
                                timeout: 60000
                              }
        ...
        if (head.mobile && isCordovaApp) {
          ...
          cordova.exec(
            ...
            function(e){dfd.reject(e)},
                   'CBLite',
                   'pull',
                   [{"dbname": TAXONOMY_DBNAME + DB_IDENTIFIER, "url":'http://'+syncUsername+':'+syncPassword+'@'+COUCHBASE_DOMAIN +':' + COUCHBASE_PORT + '/' + TAXONOMY_BUCKET}]
               );
              ...
              if(authenticatedToSyncGateway == false)
              {
                ...
                .done(
                       function (data)
                       {
                          ...
                          taxonomyDB.replicate.from(new PouchDB('http://' + COUCHBASE_DOMAIN +':' + COUCHBASE_PORT + '/' + TAXONOMY_BUCKET,
                            {
                              ajax: {
                                timeout: 60000
                              }
            ...
            if (head.mobile && isCordovaApp) {
              ...
              if(authenticatedToSyncGateway == false) {
                dao.authenticateToSyncGateway(syncUsername, syncPassword).done(function (data){
                  ...
                  localDB.sync(
                    new PouchDB('http://' + COUCHBASE_DOMAIN +':' + COUCHBASE_PORT + '/' + COUCHBASE_BUCKET,
                    {
                      ajax: {
                        timeout: 60000
                      }
              ...
              "view_searchData" : {
                    "map" : "function(doc) {" +
                          ...
                          "&& typeof (doc.category.term) != 'undefined'){" +
                            ...
                            "  var jsonObj = doc; emit(doc.title.toUpperCase(),  jsonObj ); " +
                              "}" +
              ...
              "views" : {
                "view_searchData" : {
                      "map" : "function(doc) {" +
                            ...
                            "  var jsonObj = {\"doc\": {  \"updated\": doc.updated, \"_id\": doc._id, \"title\": doc.title, \"category\": { \"term\": doc.category.term }, \"model\": { \"pending\": { \"photos\": \"\" } } }}; emit(doc.title.toUpperCase(),  jsonObj ); " +
                              "}" +
